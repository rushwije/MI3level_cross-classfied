...
for (i in 1: length(temp)){
  ...
  dataw_in <- reshape(dataL,v.names=c("school","t_rating","prev_sdq","prev_dep"),timevar = "time",idvar="c_id",direction= "wide")

  #remove school cluster indicators at waves 1,3 and 4
  dataw <- select(dataw_in,-c(school.W1,school.3,school.4,school.5,school.6,school.7,school.8))
  
  ##Set number of imputations and number of burn-in iterations
  M<-20
  
  ##create school dummy indicators
  dataw$school.2=as.factor(dataw$school.2)
  
  ##Perform imputations
  imp2=mice(data=dataw[,!names(dataw)%in%c("c_id")],m=M,method="norm",maxit=10)
  ...
  for(m in 1:M)
  {
    ...
    datw <- datw %>% select(!contains("prev_sdq")& !school.2)
    
    #1. attach school cluster variables and remove DIs
    datw=cbind(datw,dataw_in$school.2,dataw_in$school.3,dataw_in$school.4,
               dataw_in$school.5,dataw_in$school.6,dataw_in$school.7,
               dataw_in$school.8,dataw_in$c_id)
    
    
    datw=datw %>% rename(school.2=`dataw_in$school.2`,school.3=`dataw_in$school.3`,
                         school.4=`dataw_in$school.4`,school.5=`dataw_in$school.5`,school.6=`dataw_in$school.6`,
                         school.7=`dataw_in$school.7`, school.8=`dataw_in$school.8`,c_id=`dataw_in$c_id`)
...
##save the results
write.xlsx(results.est,"FCS1LDIwide.second_Results_est.xlsx")
write.xlsx(results.sd,"FCS1LDIwide.second_Results_sd.xlsx")
write.xlsx(results.RE,"FCS1LDIwide.second_Results_RE.xlsx")
write.xlsx(results.CI,"FCS1LDIwide.second_Results_CI.xlsx")
write.xlsx(results.ICC,"FCS1LDIwide.second_Results_ICC.xlsx")
