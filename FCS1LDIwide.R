...
#  Rushani Wijesuriya 23 December 2020                                          #
#                                                                               #
#     SL-FCS-wide-DI-varying with waves 2,3 and 4                                #
###################################################################################
...
for (i in 1:length(temp)){
  ...
  names <- c("school.2","school.3","school.4","school.5","school.6","school.7","school.8")
  dataw[,names] <- lapply(dataw[,names] , factor)
  ...
  #IMPORTANT: here the school cluster indicator at the post wave is used (dep.1 <- cluster 2,dep.2 <- cluster 3 etc)
  ...
  for(m in 1:M)
  {
    ...
    datL=reshape(datw,varying =list(c("prev_dep.2","prev_dep.3","prev_dep.4","prev_dep.5","prev_dep.6","prev_dep.7","prev_dep.8"),
                 ...
                 v.names=c("prev_dep","t_score","school"), times=c(2,3,4,5,6,7,8),direction= "long")
  ...
  mods <- lapply(mylist,function(d) {lmer(t_score~ prev_dep+time+c_age+t_score.W1+c_sex+c_ses+
...
##save the results
write.xlsx(results.est,"FCS1LDIwide.234_Results_est.xlsx")
write.xlsx(results.sd,"FCS1LDIwide.234_Results_sd.xlsx")
write.xlsx(results.RE,"FCS1LDIwide.234_Results_RE.xlsx")
write.xlsx(results.CI,"FCS1LDIwide.234_Results_CI.xlsx")
write.xlsx(results.ICC,"FCS1LDIwide.234_Results_ICC.xlsx")
