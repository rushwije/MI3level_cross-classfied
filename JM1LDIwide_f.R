...
for (i in 1: length(temp)){
  ...
  datacomp= cbind(Intercept=rep(1,nrow(dataw)),
                  dataw[,!names(dataw)%in%c("c_id","prev_dep.2","prev_dep.3","prev_dep.4","prev_dep.5","prev_dep.6",
                                            "prev_dep.7","prev_dep.8","school.W1","school.3",
                                            "school.4","school.5","school.6","school.7","school.8")])
  
  ##create school dummy indicators
  
  ##create school dummy indicators for waves, 2,3 and 4
  datacomp_dummy=fastDummies::dummy_cols(datacomp, select_columns =c("school.2"),
                                         remove_first_dummy = TRUE)
  
  #remove school cluster indicators 
  datacomp_dummy <- datacomp_dummy %>% select(-c("school.2"))
  ...
  for(m in 1:M)
  {
    ...
    datw=datw %>% select(!contains(c("school.2_","prev_sdq")),-Imputation,-Intercept)
...
##save the results
write.xlsx(results.est,paste0("JM1LDIwide.second_Results_est",datnum,".xlsx"))
write.xlsx(results.sd,paste0("JM1LDIwide.second_Results_sd",datnum,".xlsx"))
write.xlsx(results.RE,paste0("JM1LDIwide.second_Results_RE",datnum,".xlsx"))
write.xlsx(results.CI,paste0("JM1LDIwide.second_Results_CI",datnum,".xlsx"))
write.xlsx(results.ICC,paste0("JM1LDIwide.second_Results_ICC",datnum,".xlsx"))
